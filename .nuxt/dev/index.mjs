...
const settings_post = defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (type === "course") {
      ...
      const existing = await db.select().from(courses$1).where(eq(courses$1.slug, slug)).limit(1);
      ...
      await db.insert(courses$1).values(newCourse);
    ...
    if (type === "course-update") {
      ...
      const existing = await db.select().from(courses$1).where(eq(courses$1.id, courseId)).limit(1);
      if (existing.length === 0) {
        throw createError({ statusCode: 404, message: "\u041A\u0443\u0440\u0441 \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D" });
      }
      if (slug && slug !== existing[0].slug) {
        const slugExists = await db.select().from(courses$1).where(eq(courses$1.slug, slug)).limit(1);
      ...
      await db.update(courses$1).set(updateData).where(eq(courses$1.id, courseId));
      const [updatedCourse] = await db.select().from(courses$1).where(eq(courses$1.id, courseId)).limit(1);
    ...
    if (type === "course-delete") {
      ...
      const existing = await db.select().from(courses$1).where(eq(courses$1.id, data.id)).limit(1);
      if (existing.length === 0) {
        throw createError({ statusCode: 404, message: "\u041A\u0443\u0440\u0441 \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D" });
      }
      await db.delete(courses$1).where(eq(courses$1.id, data.id));
